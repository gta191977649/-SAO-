...
/// </summary>
    class LyricParser
    {
        ...
        /// <summary>
        /// 返回当前的时间,Episodes写的函数
        /// </summary>
        public ArrayList TimeArray
        {
            get
            {
                return timeArray;
        ...
        protected static double stringToInterval(String str)
        {
            ...
            {
                double min,sec;
                if (double.TryParse(str.Split(':').GetValue(0).ToString(), out min)) min = double.Parse(str.Split(':').GetValue(0).ToString());
                if (double.TryParse(str.Split(':').GetValue(1).ToString(), out sec)) sec = double.Parse(str.Split(':').GetValue(1).ToString());
